...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_CONFIG_PARAM_ID[APP_CONFIG_PARAM_ID_LEN] =                          
#else
...
#ifdef SW_UC_PIC18F
 //const rom uint8 APP_CONFIG_PARAM_TYPE[APP_CONFIG_PARAM_TYPE_LEN] =                      
 #ifdef SW_PROD_Rf
   #define APP_CONFIG_PARAM_TYPE_LEN       2
   const rom uint8 APP_CONFIG_PARAM_TYPE[APP_CONFIG_PARAM_TYPE_LEN] =
                                                                                    {'r','f'};
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_CONFIG_PARAM_STATUS_CONFIG[APP_CONFIG_PARAM_STATUS_CONFIG_LEN] =    
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_CONFIG_PARAM_PULSES[APP_CONFIG_PARAM_PULSES_LEN] =                  
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_CONFIG_PARAM_VERSION[APP_CONFIG_PARAM_VERSION_LEN] =                
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_EVENT_PARAM_ON[APP_EVENT_PARAM_ON_LEN] =                            
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_EVENT_PARAM_OFF[APP_EVENT_PARAM_OFF_LEN] =                          
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_EVENT_PARAM_NO[APP_EVENT_PARAM_NO_LEN] =                            
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_EVENT_PARAM_YES[APP_EVENT_PARAM_YES_LEN] =                            
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_ID_MOTORX[APP_TECHINFO_PARAM_ID_MOTORX_LEN] =        
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_ID_MOTORY[APP_TECHINFO_PARAM_ID_MOTORY_LEN] =        
#else
...
#define APP_TECHINFO_PARAM_ID_RGBLED_LEN          6
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_ID_RGBLED[APP_TECHINFO_PARAM_ID_RGBLED_LEN] =        
#else
 const uint8 APP_TECHINFO_PARAM_ID_RGBLED[APP_TECHINFO_PARAM_ID_RGBLED_LEN] =        
  ...
  const rom uint8 APP_TECHINFO_PARAM_ID_APP[APP_TECHINFO_PARAM_ID_APP_LEN] =        
                                                                                    {'T','r','i','P','o','l','l','a','r'};
 #endif
 #ifdef SW_PROD_Us
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_ID_5V[APP_TECHINFO_PARAM_ID_5V_LEN] =        
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_ID_12V[APP_TECHINFO_PARAM_ID_12V_LEN] =        
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_ID_TEMP[APP_TECHINFO_PARAM_ID_TEMP_LEN] =        
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_ID_CUR[APP_TECHINFO_PARAM_ID_CUR_LEN] =        
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_ID_TIPCOUNT[APP_TECHINFO_PARAM_ID_TIPCOUNT_LEN] =        
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_ID_TIPID[APP_TECHINFO_PARAM_ID_TIPID_LEN] =        
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_ID_STATUS[APP_TECHINFO_PARAM_ID_STATUS_LEN] =        
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_ID_PULSES[APP_TECHINFO_PARAM_ID_PULSES_LEN] =        
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_STATUS_OP[APP_TECHINFO_PARAM_STATUS_OP_LEN] =        
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_STATUS_BRK[APP_TECHINFO_PARAM_STATUS_BRK_LEN] =      
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_REASON_PASS[APP_TECHINFO_PARAM_REASON_PASS_LEN] =    
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_REASON_FAIL[APP_TECHINFO_PARAM_REASON_FAIL_LEN] =    
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_REASON_FIRM[APP_TECHINFO_PARAM_REASON_FIRM_LEN] =    
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_REASON_PULSES[APP_TECHINFO_PARAM_REASON_PULSES_LEN] =    
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TECHINFO_PARAM_REASON_TIPID[APP_TECHINFO_PARAM_REASON_TIPID_LEN] =    
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TIPINFO_PARAM_DISCONNECTED[APP_TIPINFO_PARAM_DISCONNECTED_LEN] =        
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_TIPINFO_PARAM_CONNECTED[APP_TIPINFO_PARAM_CONNECTED_LEN] =        
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_EVENT_FAULT_CODE_OVRVOLT[APP_EVENT_FAULT_CODE_OVRVOLT_LEN] =                          
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_EVENT_FAULT_CODE_TEMP[APP_EVENT_FAULT_CODE_TEMP_LEN] =                          
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_EVENT_FAULT_CODE_MOTX[APP_EVENT_FAULT_CODE_MOTX_LEN] =                          
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_EVENT_FAULT_CODE_MOTY[APP_EVENT_FAULT_CODE_MOTY_LEN] =                          
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_EVENT_FAULT_CODE_RGBLED[APP_EVENT_FAULT_CODE_RGBLED_LEN] =                          
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_EVENT_FAULT_CODE_OVRCUR[APP_EVENT_FAULT_CODE_OVRCUR_LEN] =                          
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_EVENT_FAULT_CODE_RF[APP_EVENT_FAULT_CODE_RF_LEN] =                          
#else
...
#ifdef SW_UC_PIC18F
 const rom uint8 APP_EVENT_FAULT_CODE_TIP[APP_EVENT_FAULT_CODE_TIP_LEN] =                          
#else
